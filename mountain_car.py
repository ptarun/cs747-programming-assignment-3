...
class sarsaAgent():
    ...
    def __init__(self):
        ...
        self.epsilon_T1 = 0.4
        self.epsilon_T2 = 0.5
        self.learning_rate_T1 =  0.35
        self.learning_rate_T2 = 0.3
        '''
        best value = -146.16
        epsilon_T1 = 0.4               
        learning_rate_T1 =  0.35
        n_p = 100
        n_v = 80
        weight initial = zeros
        '''
        self.n_p = 100
        self.n_v = 80
        ...
        self.n_v_T2 = 55
        self.n_tiles = 3
        self.n_grid = self.n_p_T2 * self.n_v_T2

        self.weights_T1 = [np.zeros((self.n_p,self.n_v,3))] # newly added
        #self.weights_T2 = np.zeros((self.n_grid,self.n_grid,self.n_grid,3))
        self.weights_T2 = [np.zeros((self.n_p_T2,self.n_v_T2,3)) for i in range(self.n_tiles)]
        #self.weights_T2 = [np.random.rand(self.n_p_T2, self.n_v_T2, 3) for i in range(self.n_tiles)]
        ...
        #print(self.epsilon_T2,self.learning_rate_T2,self.n_p_T2,self.n_v_T2,self.n_tiles)
    ...
    def get_table_features(self, obs):
        ...
        st = np.array([obs[i] + abs(self.lower_bounds[i]) for i in range(len(obs))])

        high = np.array([x + abs(y) for x, y in zip(self.upper_bounds, self.lower_bounds)])

        new_p = int(st[0] / (high[0] / self.n_p))
        new_v = int(st[1] / (high[1] / self.n_v))

        new_state = [(new_p, new_v)]
    ...
    # it will take around 15 min to run
    def get_better_features(self, obs):

        st = np.array([obs[i] + abs(self.lower_bounds[i]) for i in range(len(obs))])
        new_state = [(0,0) for i in range(self.n_tiles)]
        high = np.array([x + abs(y) for x, y in zip(self.upper_bounds, self.lower_bounds)])

        #high_phi = np.array(np.exp())

        pos = np.linspace(0, high[0], self.n_p_T2)
        vel = np.linspace(0, high[1], self.n_v_T2)

        #tile coding
        offsets = [0.02, 0.02]
        for i in range(self.n_tiles):
            pos = [p + i*offsets[0] for p in pos]
            vel = [v + i*offsets[1] for v in vel]

            new_p = np.digitize(st[0], pos)
            new_v = np.digitize(st[1], vel)

            new_state[i] = (new_p, new_v)
    ...
    def choose_action(self, state, weights, epsilon):
        ...
        action_list = np.zeros(self.env.action_space.n)
        for act in range(self.env.action_space.n):
            #print(state,act)
            #action_list[act] = 0
            if(len(weights)==1):
                #print(state)
                action_list[act] = weights[0][state[0]][act]
            else:
                action_list[act] = sum([weights[tile][state[tile]][act] for tile in range(self.n_tiles)])
    ...
    def sarsa_update(self, state, action, reward, new_state, new_action, learning_rate, weights):
        ...
        for tile in range(len(weights)):

            target = reward + self.discount * weights[tile][new_state[tile]][new_action]
            weights[tile][state[tile]][action] = weights[tile][state[tile]][action] + learning_rate * (target - weights[tile][state[tile]][action])
        return weights
...
if __name__ == "__main__":
    ...
    np.random.seed(0)    # changed order of seed
    agent = sarsaAgent()
    agent.env.seed(0)
